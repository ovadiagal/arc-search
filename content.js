...
function displayResults(results, query = "") {
  ...
  if (grouped.google.length > 0) {
    ...
    grouped.google.forEach((result, index) => {
  ...
  if (grouped.tab.length > 0) {
    html +=
      '<div class="arc-search-group"><div class="arc-search-group-title">Open Tabs</div>';
    grouped.tab.forEach((result, index) => {
      html += renderResult(result, grouped.google.length + index);
    });
    html += "</div>";
  }
  ...
  if (grouped.bookmark.length > 0) {
    html +=
      '<div class="arc-search-group"><div class="arc-search-group-title">Bookmarks</div>';
    grouped.bookmark.forEach((result, index) => {
      html += renderResult(
        result,
        grouped.google.length + grouped.tab.length + index,
      );
    });
    html += "</div>";
  }
  ...
  if (grouped.history.length > 0) {
    html +=
      '<div class="arc-search-group"><div class="arc-search-group-title">History</div>';
    grouped.history.forEach((result, index) => {
      html += renderResult(
        result,
        grouped.google.length +
          grouped.tab.length +
          grouped.bookmark.length +
          index,
      );
    });
    html += "</div>";
...
function moveSelection(delta) {
  selectedIndex = Math.max(
    0,
    Math.min(allResults.length - 1, selectedIndex + delta),
  );
